...
__weak uint8_t ssd1306_interface_iic_init(void)
{
    return 0;
}

__weak uint8_t ssd1306_interface_iic_deinit(void)
{
    return 0;
}

__weak uint8_t ssd1306_interface_iic_write(uint8_t addr, uint8_t reg, uint8_t *buf, uint16_t len)
{
    return 0;
}

__weak uint8_t ssd1306_interface_spi_init(void)
{
    return 0;
}

__weak uint8_t ssd1306_interface_spi_deinit(void)
{
    return 0;
}

__weak uint8_t ssd1306_interface_spi_write_cmd(uint8_t *buf, uint16_t len)
{
    return 0;
}

__weak uint8_t ssd1306_interface_spi_cmd_data_gpio_init(void)
{
    return 0;
}

__weak uint8_t ssd1306_interface_spi_cmd_data_gpio_deinit(void)
{
    return 0;
}

__weak uint8_t ssd1306_interface_spi_cmd_data_gpio_write(uint8_t value)
{
    return 0;
}

__weak uint8_t ssd1306_interface_reset_gpio_init(void)
{
    return 0;
}

__weak uint8_t ssd1306_interface_reset_gpio_deinit(void)
{
    return 0;
}

__weak uint8_t ssd1306_interface_reset_gpio_write(uint8_t value)
{
    return 0;
}

__weak void ssd1306_interface_delay_ms(uint32_t ms)
{
}
...
uint8_t ssd1306_advance_init(ssd1306_interface_t interface, ssd1306_address_t addr)
{
    ...
    res = ssd1306_set_column_address_range(&gs_handle, SSD1306_ADVANCE_DEFAULT_COLUMN_ADDRESS_RANGE_START,
                                           SSD1306_ADVANCE_DEFAULT_COLUMN_ADDRESS_RANGE_END);
    if (res != 0) {
        (void)ssd1306_deinit(&gs_handle);
        return 1;
    }
    ...
    res = ssd1306_set_page_address_range(&gs_handle, SSD1306_ADVANCE_DEFAULT_PAGE_ADDRESS_RANGE_START,
                                         SSD1306_ADVANCE_DEFAULT_PAGE_ADDRESS_RANGE_END);
    if (res != 0) {
        (void)ssd1306_deinit(&gs_handle);
        return 1;
    }
    ...
    res = ssd1306_set_fade_blinking_mode(&gs_handle, SSD1306_ADVANCE_DEFAULT_FADE_BLINKING_MODE,
                                         SSD1306_ADVANCE_DEFAULT_FADE_FRAMES);
    if (res != 0) {
        (void)ssd1306_deinit(&gs_handle);
        return 1;
    }
    ...
    /* set display clock */
    res = ssd1306_set_display_clock(&gs_handle, SSD1306_ADVANCE_DEFAULT_OSCILLATOR_FREQUENCY,
                                    SSD1306_ADVANCE_DEFAULT_CLOCK_DIVIDE);
    if (res != 0) {
        (void)ssd1306_deinit(&gs_handle);
        return 1;
    }
    ...
    res = ssd1306_set_precharge_period(&gs_handle, SSD1306_ADVANCE_DEFAULT_PHASE1_PERIOD,
                                       SSD1306_ADVANCE_DEFAULT_PHASE2_PERIOD);
    if (res != 0) {
        (void)ssd1306_deinit(&gs_handle);
        return 1;
    }
    ...
    res = ssd1306_set_com_pins_hardware_conf(&gs_handle, SSD1306_ADVANCE_DEFAULT_PIN_CONF,
                                             SSD1306_ADVANCE_DEFAULT_LEFT_RIGHT_REMAP);
    if (res != 0) {
        (void)ssd1306_deinit(&gs_handle);
        return 1;
    }
